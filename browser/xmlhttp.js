...
function do_browser(){
      ...
      if(loadXMLDoc("index.php?option=com_xcloner-backupandrestore&task=files_xml&nohtml=1")){
...
function useXML(xmlInfo){
      ...
      var newloc1="index.php?option=com_xcloner-backupandrestore&task=files_xml&nohtml=1&dir=" + loc ;

      var stringHTML="<p><b>" + loc1 +"</b> <br /><br /></p>";

      if(loc.indexOf("/")!=-1){
      var newfile="index.php?option=com_xcloner-backupandrestore&task=files_xml&nohtml=1&dir=" + loc.slice(0, loc.lastIndexOf("/"));
      ...
      if(infoTags[0].hasChildNodes){
            ...
            for(node=infoTags[0].firstChild; node!=null; node=node.nextSibling){
                  ...
                  var newloc="index.php?option=com_xcloner-backupandrestore&task=files_xml&nohtml=1&dir=" + loc ;
                  path = loc+"/"+node.firstChild.nodeValue;

                  if(node.nodeName=="file"){

                        stringHTML=stringHTML+"<li class=\"file\"><img src='components/com_xcloner-backupandrestore/browser/file.gif' border='0'><input type=checkbox "+node.getAttribute('check')+" onclick=\"loadXMLDoc('" + newloc +"&amp;path="+path+"')\" name=cid[] value='"+path+"'><a href=\"" +node.getAttribute('link')+ "\">"+node.firstChild.nodeValue+"</a></li>";
                  ...
                  if(node.nodeName=="folder"){
                        ...
                        var newfile="index.php?option=com_xcloner-backupandrestore&task=files_xml&nohtml=1&dir=" + loc + "/" + node.firstChild.nodeValue;
                        stringHTML=stringHTML+"<li class=\"folder\"><img src='components/com_xcloner-backupandrestore/browser/folder.gif' border='0'><input type=checkbox "+node.getAttribute('check')+" onclick=\"loadXMLDoc('" + newloc +"&amp;path="+path+"')\" name=cid[] value='"+path+"'><a href='#' class=\"folder\" onclick=\"loadXMLDoc('" + newfile+"')\">"+node.firstChild.nodeValue+"</a></li>";
                  }
            }
            stringHTML=stringHTML+"</ul>";
      }
